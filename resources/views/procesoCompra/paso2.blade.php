...
<div class="mx-auto mt-20 sm:mt-24 lg:mt-32 px-4 sm:px-6 lg:px-8 py-6 max-w-lg sm:max-w-xl lg:max-w-2xl text-white font-primary-font bg-black/20 rounded-lg backdrop-blur">
    ...
    <div class="mb-8 space-y-4 text-base sm:text-lg">
        <p class="flex items-center gap-2 break-words">
            ...
            <span class="font-semibold">Película:</span> {{ $infoPelicula->pelicula->titulo }}
        </p>
        ...
        <p class="flex items-center gap-2">
            ...
            <span class="font-semibold">Total:</span> {{ $total }}€
        </p>
    </div>
    ...
    <div class="mb-8">
        ...
        <ul class="list-disc list-inside pl-4 space-y-1 text-sm sm:text-base">
            @foreach(json_decode($butacas, true) as $butaca)
                <li>{{ $butaca }}</li>
            @endforeach
        </ul>
    </div>

    <div class="flex flex-col sm:flex-row justify-between gap-4 my-10">
        <a href="{{ route('procesoCompra.paso1', ['idSesion' => $infoPelicula->id]) }}" class="flex items-center justify-center gap-2 px-4 py-2 rounded border border-white text-white hover:bg-white/80 hover:text-gray-800 transition">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 sm:w-6 sm:h-6">
